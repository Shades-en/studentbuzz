...
const SidebarStud = () => {
    ...
    function changeActive() {
        ...
        if(url.includes('notif')) {
            nav = {feed: false, clubs: false, notif: true, search: false}
        }
        if(url.includes('search')) {    
            nav = {feed: false, clubs: false, notif: false, search: true}
        }
        if(url.includes('clubs')) {
            nav = {feed: false, clubs: true, notif: false, search: false}
        }
        if(url.includes('feed')) {
            nav = {feed: true, clubs: false, notif: false, search: false}
        }
        var current
            ...
            <div className="min-h-screen w-52 z-10 flex flex-row bg-gray-100 fixed left-0 shadow-2xl" id="nav_container">
                <div className="flex flex-col mt-5 w-full">
                    ...
                    <li >
                        ...
                        <span className="text-sm font-medium hover:#6366f1 flex items-center justify-start"> <FeedOutlinedIcon fontSize="small" className="text-indigo-500 mr-3"/> <Link to="/student/feed" onClick={handleActive} id="feed" >Feed</Link>
                        </span>
                        </a>
                    </li>
                    <li>
                        ...
                        <span className="inline-flex items-center justify-center h-12 w-12 text-lg text-gray-400"></span>
                        <span className="text-sm font-medium hover:#6366f1 flex items-center justify-start"> <EmailOutlinedIcon fontSize="small" className="text-indigo-500 mr-3"/> <Link to="/student/notif" onClick={handleActive} id="notif">Notifications</Link> </span>
                        </a>
                    </li>
                    <li>
                        <a href="#" className="sidebar-cat flex flex-row items-center h-12 transform hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800">
                    ...
                    <li>
                        <a href="#" className="sidebar-cat flex flex-row items-center h-12 transform hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800">
                        <span className="inline-flex items-center justify-center h-12 w-12 text-lg text-gray-400"></span>
                        <span className="text-sm font-medium hover:#6366f1 flex items-center justify-start"> <Diversity1OutlinedIcon fontSize="small" className="text-indigo-500 mr-3"/> <Link to="/student/clubs" onClick={handleActive} id="clubs"> Clubs</Link> </span>
                        </a>
                    </li>
                    </ul>
                </div>
